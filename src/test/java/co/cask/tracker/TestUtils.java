...
public final class TestUtils {
  ...
  public static String getServiceResponse(ServiceManager serviceManager, String request, String type,
    ...
    if (type.equals("POST")) {
      httpResponse = HttpRequests.execute(HttpRequest.post(url).withBody(postRequest).build());
    } else if (type.equals("PUT")) {
      httpResponse = HttpRequests.execute(HttpRequest.put(url).withBody(postRequest).build());
    }
    Assert.assertEquals(expectedResponseCode, httpResponse.getResponseCode());
    if (httpResponse.getResponseCode() == HttpURLConnection.HTTP_OK ||
      expectedCodes.contains(httpResponse.getResponseCode())) {
      return httpResponse.getResponseBodyAsString();
    } else {
      throw new Exception("Invalid response code returned: " + httpResponse.getResponseCode());
    }
  }
